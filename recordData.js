...
async function getLastIndex() {
  try {
    ...
    if (latestBlockHeight >= 1) {
      ...
      console.log("Starting with block number:===" + 55225301);
    }
...
async function listenToBlocks() {
  try {
    ...
    if (
      (blockNum !== 55225301 && blockNum == 0) ||
      blockNum == null ||
      blockNum == 1
    ) {
      blockNum = 55225301; // decided index to start fetching data from
    }
    ...
    while (true) {
      try {
          ...
          try {
            ...
            for (const txs of block.transactions) {
              ...
              db.run(txInsertSql, [
                ...
                transactionData.stakedAssetReleasedBalance,
        ...
        // making a Json file for the tron Data
